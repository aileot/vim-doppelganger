...
let s:Text = {}
function! doppelganger#format#new(pair_info) abort
...
function! s:Text__read_contents() abort dict
  const Contents = self.is_reverse
        \ ? doppelganger#format#contents#new(self.corr_lnum)
        \ : doppelganger#format#contents#new(self.corr_lnum, self.curr_lnum)
...
function! s:Text__compose_chunks(contents) abort dict
  let Components = doppelganger#format#components#new(
        \ self.curr_lnum,
        \ self.corr_lnum,
        \ )
